    ...
    <form wire:submit.prevent="addComment">
        <div class="col-span-4 flex items-center space-x-2">
            ...
            <div class="w-full col-span-4 border-none shadow-none bg-none border-white">
                <x-input.text wire:model.lazy="newComment" class="" placeholder="Add a comment..."  :error="$errors->first('newComment')"/>
            </div>
        </div>
        <div class="flex justify-end opacity-50 text-insta text-xl">
            <x-button.link type="submit" class="text-xl tracking-wider" ><strong>Post</strong></x-button.link>
        </div>
    </form>
    @foreach($comments as $comment)
        <div class="rounded-lg border shadow p-3 my-2">
